...
export default {
  ...
  config: {
    hideCurrentProjectInHistory: {
      type: 'boolean',
      default: true,
    },
    reloadPkgs: {
      type: 'array',
      default: ['tabs', 'find-and-replace'],
      items: {
        type: 'string',
      },
    },
  ...
  toggle() {
    if (this.view.isVisible()) {
      ...
      const items = atom.history.projects.map(({ _paths: paths }) => ({
        paths,
        name: paths.map(i => path.basename(i)).join(', '),
        info: paths.map(fs.tildify).join(', '),
      }))
      if(atom.config.get('project-neue.hideCurrentProjectInHistory')) {
        _.remove(items, i => _.isEqual(i.paths, atom.project.getPaths()))  
      }
